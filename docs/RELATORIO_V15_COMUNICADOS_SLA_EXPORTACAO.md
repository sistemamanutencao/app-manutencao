# Relatório v15 — Comunicados, SLA e exportação de OS
...
Esta versão consolida a evolução aprovada a partir da base funcional v13 e não altera a arquitetura Firebase Spark validada.
...
## Correção operacional consolidada na v15
...
## Ajuste complementar consolidado na v15 — filtros e exportação de Ordens de Serviço

Na consolidação da v15, foram adicionados ajustes na aba **Ordens de Serviço** para melhorar consulta e extração de relatórios pela Manutenção.

Alterações adicionadas:

- Inclusão do filtro **Encerrados** junto aos filtros existentes:
  - Todos;
  - Atrasados;
  - Abertos;
  - Andamento;
  ...
  - Cancelados;
  - Encerrados.
- Atualização do card de exportação para **Exportar Ordens de Serviço**, removendo a limitação conceitual de OS finalizadas.
- Exportação agora considera as OS disponíveis para o perfil Manutenção de acordo com o filtro e a busca ativos na tela.
- Se o filtro estiver em **Todos**, exporta todos os chamados carregados para Manutenção.
- Se o filtro estiver em um status específico, exporta os chamados daquele status.
- Se o filtro estiver em **Atrasados**, exporta apenas OS atrasadas.
- A exportação continua restrita ao perfil Manutenção pela classe `manut-only`.
- Não houve alteração em `firestore.rules` para esta melhoria, pois a exportação usa dados que a Manutenção já pode ler.
...
- `docs/RELATORIO_V15_COMUNICADOS_SLA_EXPORTACAO.md`
...
- Abrir a aba Ordens de Serviço.
- Confirmar presença do filtro Encerrados.
- Confirmar que Encerrados lista apenas OS com status `ENCERRADO`.
- Testar exportação em Todos, Atrasados, Abertos, Cancelados e Encerrados.
- Confirmar que o arquivo exportado respeita o filtro e a busca ativos.
- Confirmar que a exportação continua visível apenas para Manutenção.
